           ...
           #Food loop
            while True:
                ...
                if food=="full":
                    ...
                    item_count+=1
                #print("good3")
        #print("before recommend list")
        #print(protein_goal - current_protein, carb_goal - current_carb, fat_goal - current_fat)
...
def Data_handling(input, output,key):
    ...
    summarized_data=data.groupby(data[key].apply(lambda x: split(r"(\,| )", x)[0])).mean(numeric_only=True).reset_index().round(2)
